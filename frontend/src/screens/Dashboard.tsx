...
import {
  BedDouble, LogIn, Users, CircleDollarSign, Wallet, Building2, Brush, Receipt,
        ...
        <>
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6">
            ...
            <Tile icon={BarChart3} label={t("RevPAR")} value={`${cur()}${inr(prop.statistics.revpar)}`}
              sub={t("month to date")} />
            <Tile icon={CircleDollarSign} label={t("Revenue")} value={`${cur()}${inr(prop.revenue_today)}`}
              sub={t("today")} />
      ...
      {scope === "portfolio" && port && (
        <>
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6">
            ...
            <Tile icon={CircleDollarSign} label={t("Revenue")} value={`${cur()}${inr(port.totals.revenue_today)}`} sub={t("today")} />
